...
"${SSH_BASE[@]}" "bash -s" -- "$VERSION" "$TEST_ROOT" "$REMOTE_STAGE" "$INSTALLER" "${INSTALLER_BINS[0]}" "${INSTALLER_BINS[1]}" "$PORTABLE" <<'REMOTE'
set -euo pipefail
VERSION="$1"
ROOT="$2"
STAGE="$3"
...
for file in "$INSTALLER" "$INSTALLER_BIN_0" "$INSTALLER_BIN_1" "$PORTABLE" release-notes.txt TESTING-ONLY.txt "SHA256SUMS-${VERSION}.txt"; do
  test -s "$STAGE/$file"
done
(
  cd "$STAGE"
