...
# 파일 연결 체크는 지원 형식을 ‘연결 프로그램’ 후보로 등록하는 역할만 한다.
# Windows UserChoice 기본 앱 설정 화면은 설치 뒤 자동으로 열면 안 된다.
for forbidden in ('--register-file-associations', 'ms-settings:defaultapps'):
    if forbidden in installer:
        errors.append(f"파일 연결 뒤 자동 기본 앱 설정 화면 경로가 남아 있습니다: {forbidden}")
